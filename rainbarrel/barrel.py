...
"""
    ...
    def iso8601(self, tim=None, **params):
        if tim is None:
            tim = time.time()
    ...
    def load_state(self):
        ...
        try:
            ...
            self.log.warning("No previous state file '%s' -- %s", self.statefile, e)
            return {}
        try:
            return json.loads(prev_state)
        except Exception as e:
            self.log.warning("Parse failed on previous state file '%s' -- %s", self.statefile, e)
            return {}
    ...
    def save_state(self):
        ...
        try:
            ...
            self.log.error("Write failed to '%s' -- %s", temp, e)
            return
        try:
            os.rename(temp, self.statefile)
        except Exception as e:
            self.log.error("Rename of '%s' to '%s' failed -- %s", temp, self.statefile, e)
    ...
    """
        ...
        try:
            ...
            self.log.warning("Could not convert TimeStamp %r to clock delta -- %s", timestamp, e)
            clock_delta = ''
    ...
    """
        ...
        if fmt == 'json':
            json_params = {}
            try:
                json_params['indent'] = int(q.get('indent')[0])
            except:
                pass
    ...
    """
        ...
        try:
            ...
            self.log.error("Payload parse failed -- %s", e, exc_info=True)
        ...
        for node in dom.getElementsByTagName('rainforest'):
            ...
            for command in node.childNodes:
                ...
                for elem in command.childNodes:
                    ...
                    if elem.childNodes:
                        ...
                        val = ''
                        valstr = val
                    self.log.debug("%s %s = %r", command.nodeName, elem.nodeName, val)
                    info[tag] = val
                    raw_info[tag] = valstr
            ...
            for tag in ['_mac', '_version']:
                if header[tag] != self.state.get(tag):
                    self.log.info("Rainforest %s value changed from %r to %s", tag, self.state.get(tag), header[tag])
                    self.state[tag] = header[tag]
        for plug, name in self.plugins:
            try:
                self.log.debug("Running plugin '%s' for command '%s'", name, self.state['_last_element'])
                plug.handle(self)
            except Exception as e:
                self.log.error("Plugin '%s' failed for '%s' -- %s",
                                name, self.state['_last_element'], e, exc_info=True)
        self.save_state()
    ...
    """
        ...
        try:
                    ...
                    if event_cnt > 0:
                        ...
                        for item, mask in evlist:
                            if item == httpd_server:
                                ...
                                raise Exception("Unknown poll item %r" % item)
                    else:
                        self.log.debug("Idle")
                        self.event_set.idle()
        except Exception as e:
            self.log.error("Unexpected event -- %s", e, exc_info=True)
        finally:
            if event_listen:
                try:
                    event_listen.close()
            ...
            try:
                ...
                self.log.warning("Error closing htto server -- %s", e)
    ...
    """
        ...
        try:
            ...
            self.log.debug("Scan of plugin dir '%s' failed -- %s", dir, e)
            ...
            try:
                ...
                for elem, obj in inspect.getmembers(m):
                    ...
                    try:
                        ...
                        self.log.warning("Class instantiation of '%s' failed -- %s", name, e)
            except Exception as e:
                self.log.warning("Module load of '%s' failed -- %s", name, e)
        return plugins
